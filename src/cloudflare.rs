...
impl Client {
    ...
    fn get<T>(&self, opts: RequestOptions) -> eyre::Result<T>
    where
        T: serde::de::DeserializeOwned,
    {
        ...
        let url = self.url.join(&endpoint).wrap_err_with(|| {
            format!("Joining endpoint {} with base URL {}", endpoint, self.url)
        })?;
    ...
    pub async fn download_async(&self, size: usize) -> eyre::Result<MeasurementResult> {
        tokio::task::spawn_blocking({
            let this = self.clone();
            move || this.download(size)
        })
        .await
        .wrap_err_with(|| "Task panicked")?
    }

    pub fn download(&self, size: usize) -> eyre::Result<MeasurementResult> {
        self.measure_request(RequestOptions::get(format!("/__down?bytes={}", size)))
    }

    pub async fn upload_async(&self, size: usize) -> eyre::Result<MeasurementResult> {
        tokio::task::spawn_blocking({
            let this = self.clone();
            move || this.upload(size)
        })
        .await
        .wrap_err_with(|| "Task panicked")?
    }

    pub fn upload(&self, size: usize) -> eyre::Result<MeasurementResult> {
    ...
    fn measure_request(&self, opts: RequestOptions) -> eyre::Result<MeasurementResult> {
...
impl Default for Client {
    fn default() -> Self {
        Self {
            url: Arc::new(
                reqwest::Url::parse(SPEED_URL)
                    .unwrap_or_else(|_| panic!("Parsing URL {}", SPEED_URL)),
            ),
        }
    }
}
